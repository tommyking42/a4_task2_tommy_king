---
title: "Assignment 4 Task 2"
author: "Tommy King"
date: "11/7/2021"
output: 
  html_document: 
    toc: yes
    number_sections: yes
---

```{r setup, include=TRUE, message = FALSE, warning = FALSE}
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning = FALSE)

library(tidyverse)
library(here)
library(janitor)
library(lubridate)
library(dplyr)
library(kableExtra)
library(plotly)
library(broom)
library(equatiomatic)

```

```{r}
# reading in the data 
bonanza_hares <- read_csv(here("data", "bonanza_hares.csv")) %>% 
  clean_names() %>% 
# clean the date up using lubridate 
mutate(date = mdy(date)) %>% 
  mutate(year = year(date)) %>% 
# isolating juvenile hares 
filter(age %in% c("j", "J")) 
```


# Introduction: 

# Data and Analyses: 

# Juvenile Hare Initial Investigation: 

## Annual Juvenile Hare Trap Counts

```{r}
hare_trap_counts <- bonanza_hares %>% 
  filter(age %in% c("j","J")) %>%
  group_by(year) %>% 
  summarize(trap_total = n())

# visualization of trapped juveniles by year 
ggplot(data = hare_trap_counts, 
       aes(x = year, y = trap_total)) + 
  geom_histogram(fill = "palegreen", 
           stat  = "identity", 
           width = 0.5) + 
  scale_x_discrete(limit = c(1999:2012)) + 
  theme_minimal() + 
  labs(title = "Juvenile Hares Trapped at Bonanza Creek per Year", 
       y = "Number of Trapped Hares", 
       x = "Year", 
       caption = "Figure 1: Juvenile Snowshoe Hares Trapped: Number of juvenile Snowshoe Hares
       trapped per year at the Bonanza Creek Trapping Site from 1998 to 2012.") +
  theme(axis.text.x = element_text(angle = 60, hjust = 1)) + 
  theme(plot.title = element_text(hjust = 0, size = 20))
```

```{r, include = FALSE}
# Creating important descriptive stats to analyze data 
hare_traps_data <- hare_trap_counts %>% 
  summarize(mean = round(mean(trap_total), 0), 
            max = max(trap_total), 
            min = min(trap_total), 
            st_dev = sd(trap_total))
```


### Annual Juvenile Trapped Snowshoe Takeaways: 
Over the years from 199 to 2012, there is a clear decline in the amount of hares trapped as depicted by our graph in Figure 1. Based on our descriptive data, the mean amount of hares trapped yearly was 32, however, that is significantly affected by our outliers in which the maximum trappings in one year was 126 while the minimum was 2. As a result, it is hard to gauge whether this data is a result of less time spent in the field, less traps set out in the field, or a decline in the juvenile Snowshoe Hare population. In order to standardize this process for future analyses, the amount of traps set and time spent in the field should be constant for every year, which will allow scientists to determine whether or not the juvenile hare population is declining more accurately. 

# Visualizing Juvenile Hare Weights 

```{r}
# wrangling the data to a simpler dataframe
hare_weights_sex <- bonanza_hares %>% 
  select(grid, sex, weight) %>% 
  mutate(sex = case_when(
    sex %in% "f" ~ "Female", 
    sex %in% "m" ~ "Male", 
    TRUE ~ "NA"
  )) %>% 
mutate(grid = case_when(
  grid %in% "bonbs" ~ "Bonanza Black Spruce", 
  grid %in% "bonmat" ~ "Bonanza Mature", 
  grid %in% "bonrip" ~ "Bonanza Riparian"
))
```


```{r}
# visualizing the data 
ggplot(data = hare_weights_sex, 
       aes(x = weight)) + 
  geom_histogram(aes(fill = sex), 
                 bins = 12, 
                 color = "white") + 
  facet_wrap(~grid, scales = 'free') + 
  labs(x = "Weight (g)", 
       y = "Count of Hares", 
       title = "Comparison of Hares by Sex, Weight, and Site", 
       caption = "Figure 2: Juvenile Hare Weight Comparison by sex and site: Juvenile Hare Weight (g)
       comparison by site and sex. There were far more female samples taken, making it appear as if 
       the female weights are larger, however, the weights of males and females are fairly similar 
       with means around the same place.") 
 
  
```
## Main Takeaways from Visualization 
Based on the data visualization, we can surmise a few different important takeaways. Namely, in the Bonanza Riparian, there are a large amount of samples taken compared to the other two sites. Additionally, there are significantly more female hares trapped than males. Lastly, the female and male weights are relatively similar in the spread of the data, especially in terms average weights. 

# Juvenile Weight Comparison (male and female snowshoe hares) 


```{r}
#wrangling the data to include significant data statistics
 hare_sex_data <- hare_weights_sex %>% 
  filter(sex %in% c("Male", "Female")) %>% 
 drop_na(weight) %>% 
  group_by(sex) %>% 
  summarize(
    weight_mean = round(mean(weight, na.rm = TRUE), 0), 
    sd_weight = round(sd(weight, na.rm = TRUE), 0 ), 
    sample_size = n()
  )
  
```

## Data Table of Juvenile Weight Comparison 
```{r}
kable(hare_sex_data, 
      caption = "Table 1: Juvenile Snowshoe Hare Weight Comparison by Sex: Depiction of relevant data for comparing male and female snowshoe hare by weights. Includes data for mean weight for each group, standard deviation of weights, and sample size.", 
      col.names = c("Mean Weight (g)", "Standard Deviation Weight (g)", "Sample Size")) %>% 
  kable_styling(
    bootstrap_options = c("bordered", "striped", "hover")
  )
```

